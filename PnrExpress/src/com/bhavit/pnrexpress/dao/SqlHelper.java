...
public class SqlHelper extends SQLiteOpenHelper {
	...
	// insert in SEARCH_HISTORY
	public void insertInSearchHistory(SearchHistory obj) {
		SQLiteDatabase db = this.getWritableDatabase();

		// 2. create ContentValues to add key "column"/value
		ContentValues values = new ContentValues();
		values.put("STATION_FROM", obj.getStationFrom());
		values.put("STATION_TO", obj.getStationTo());

		// 3. insert
		db.insert("SEARCH_HISTORY", // table
				null, // nullColumnHack
				values); // key/value -> keys = column names/ values = column
		// values
	...
	public List<SearchHistory> getSearchHistory() {
		...
		if (cursor.moveToFirst()) {
			do {
				...
				obj.setStationFrom(cursor.getString(0));
				obj.setStationTo(cursor.getString(1));
	...
	// CHECK IF PNR EXIST
		public boolean searchHistoryExist(String fromName, String fromCode, String toName, String toCode) {

			String query = "select * from SEARCH_HISTORY where STATION_FROM = '" + fromName +"- "+fromCode
					+ "' AND STATION_TO = '"+ toName +"- "+toCode +"'";
			SQLiteDatabase db = this.getWritableDatabase();
	...
	public void onCreate(SQLiteDatabase db) {
		...
		String CREATE_TABLE_SEARCH_HISTORY = "CREATE TABLE SEARCH_HISTORY( "
				+ "STATION_FROM TEXT, " + "STATION_TO TEXT)";
